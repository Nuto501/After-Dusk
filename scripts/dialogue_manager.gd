...
signal relationship_changed(stat: String, amount: int, feedback: String)
...
func show_node(node_id: String) -> void:
	...
	line_changed.emit(
		...
		str(node.get("text", "")),
		str(node.get("expression", "neutral"))
	)
...
func choose(index: int) -> void:
	...
	var feedback := str(choice.get("feedback", ""))

	for stat in effects:
		relationship_changed.emit(str(stat), int(effects[stat]), feedback)
